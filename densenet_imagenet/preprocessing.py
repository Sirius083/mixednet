...
if __name__ == '__main__':
   ...
   # image = tf.zeros((256,204,3), dtype='float32')
   # ===============================
   ...
   print('image_preprcessed',image_preprocessed)  # return: (224,224,3)
   ...
   '''
   all_name = [n.name for n in graph_def.node]
   print('all_name', all_name)
   for name in all_name:
       print(name)
   '''

   inputs  = graph.get_tensor_by_name(input_name)  # (?,224,224,3)
   outputs = graph.get_tensor_by_name(output_name) # (?,1000)
   print('inputs', inputs)
   print('outputs', outputs)
   
   with graph.as_default():
        with tf.Session(config = config) as sess:
             result =  sess.run(outputs, {inputs: image_preprocessed})
